...
const recheckDEV = require("zrender/build/babel-plugin-transform-remove-dev")
  .recheckDEV;
  ...
  commander
    ...
    .description(
      [
        ...
        egIndent +
          ...
          "# Only prepublish.",
        egIndent +
          "node build/build.js --removedev" +
          "\n" +
          descIndent +
          "# Remove __DEV__ code. If --min, __DEV__ always be removed.",
        egIndent +
          'node build/build.js --type ""' +
          "\n" +
          descIndent +
          "# Only generate `dist/echarts.js`.",
        egIndent +
          "node build/build.js --type common --min" +
          "\n" +
          descIndent +
          "# Only generate `dist/echarts.common.min.js`.",
        egIndent +
          "node build/build.js --type simple --min --lang en" +
          "\n" +
          descIndent +
          "# Only generate `dist/echarts-en.simple.min.js`.",
        egIndent +
          'node build/build.js --lang "my/lang.js" -i "my/index.js" -o "my/bundle.js"' +
          "\n" +
          descIndent +
          "# Take `<cwd>/my/index.js` as input and generate `<cwd>/my/bundle.js`," +
          "\n" +
          descIndent +
          "where `<cwd>/my/lang.js` is used as language file."
      ].join("\n")
    )
    .option(
      "-w, --watch",
    ...
    .option(
      ...
      [
        ...
        descIndent +
          "`--lang my/langDE.js` will use `<cwd>/my/langDE.js`. -o must be specified in this case.",
        descIndent +
          "`--lang /my/indexSW.js` will use `/my/indexSW.js`. -o must be specified in this case."
      ].join("\n")
    )
    .option("--release", "Build all for release")
    .option("--prepublish", "Build all for release")
    .option(
      "--removedev",
      "Remove __DEV__ code. If --min, __DEV__ always be removed."
    )
    ...
    .option("--sourcemap", "Whether output sourcemap.")
    .option(
      "--format <format>",
...
function validateLang(lang, output) {
  ...
  if (langInfo.isOuter && !output) {
    throw new Error(
      "`-o` or `--output` must be specified if using a file path in `--lang`."
    );
  }
  if (!langInfo.absolutePath || !fs.statSync(langInfo.absolutePath).isFile()) {
    throw new Error(
      `File ${
        langInfo.absolutePath
      } does not exist yet. Contribution is welcome!`
    );
  }
}
